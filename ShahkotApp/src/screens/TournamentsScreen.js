...
export default function TournamentsScreen({ navigation }) {
  ...
  const handleCreate = async () => {
    if (!form.name.trim() || !form.description.trim() || !form.venue.trim() || !form.startDate.trim()) {
      return Alert.alert('Required', 'Name, description, venue and start date are required.');
    }
    try {
      setCreating(true);
      await tournamentsAPI.createJSON({
        ...form,
        teams,
        entryFee: form.entryFee ? parseInt(form.entryFee) : null,
      });
          ...
          <View style={styles.expandedContent}>
            ...
            {item.teams && item.teams.length > 0 && (
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>{item.teams.join(', ')}</Text>
              </View>
            )}
            {item.prize && (
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>🏆 Prize</Text>
                <Text style={styles.infoValue}>{item.prize}</Text>
              </View>
            )}
            <View style={styles.infoRow}>
            ...
            <View style={styles.actionRow}>
              ...
              {canDelete && (
                <TouchableOpacity style={styles.deleteBtn} onPress={() => deleteTournament(item.id, item.name)}>
    ...
    <View style={styles.container}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.sportList}>
        {[{ key: null, label: 'All', icon: '🏆' }, ...SPORT_TYPES].map((item) => (
          <TouchableOpacity key={item.key || 'all'} style={styles.sportCircleWrap} onPress={() => setSelectedSport(item.key)}>
            ...
            <Text style={[styles.sportCircleLabel, selectedSport === item.key && styles.sportLabelActive]}>{item.label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
